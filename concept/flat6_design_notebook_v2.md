# Flat-6 2.8 Rally – Engineering Design Notebook (Revision A)
...
**Status:** Rotating assembly frozen; piston geometry revised for squish clearance

---

# Design Change Log (Rev A)

| Issue | Original | Revised | Reason |
|---|---|---|---|
| Squish clearance inconsistency | Deck clearance = 0 mm, squish described as 1.0 mm (0.5 gasket + 0.5 piston-deck) – contradictory | Piston crown recessed 0.5 mm below deck. CRC becomes 33.5 mm. Dish volume recalculated to 9.2 cc. Squish clearance = 1.0 mm (0.5 gasket + 0.5 gap) | Resolves contradiction; maintains safe piston-to-head clearance |
| Turbo specification language | Top-level spec said “twin-scroll per bank”, manifolds were single-scroll 3-into-1, K04 listed which is single-scroll | Corrected spec to single-scroll per bank. K04 and GTX2860R remain valid candidates. | Aligns specification with actual hardware chosen |
| Compression height / dish | 34 mm CH, 12 cc dish, flush deck | 33.5 mm CH, 9.2 cc dish, 0.5 mm deck gap | Keeps compression ratio at 9.5:1 with corrected squish clearance |
...
- Configuration: One single-scroll turbo per cylinder bank (two units total)
- Compressor flow target (each): 0.17 kg/s @ PR 2.0–2.2
- Candidate turbos: BorgWarner K04 or Garrett GTX2860R (single-scroll T25 housings)
...
|---|---|
| Connecting rods | Forged 4340 steel H-beam |
| Pistons | 4032 alloy, anodized, standard pin |
| Crankshaft | Forged 4340, ion-nitrided |
| Block bore surface | Dry iron liners (Darton sleeves) |
| Valvetrain | Solid bucket tappet, shim adjustment |
| Oil system | Deep-sump wet sump with baffles |
| Turbochargers | BorgWarner K04 / GTX2860R (no e-assist) |
| ECU | rusEFI open-source |
...
- D07: Turbo layout → Single-scroll per bank (early twin-scroll concept discarded)
- D08: Piston deck clearance → Corrected to −0.5 mm, CH = 33.5 mm

---

# 7. Piston & Ring Pack Design (Corrected)
...
| Piston crown target position | 0.5 mm below deck (219.5 mm from crank CL) |
| Compression height | 219.5 − 186.0 = 33.5 mm |

## 7.2 Clearance Volume & Piston Dish

| Component | Volume (cc) |
|---|---|
| Target V_clear for CR 9.5 | 465.5 / (9.5 − 1) = 54.76 |
| Cylinder head chamber (pent-roof 4V) | 40.0 |
| Head gasket (bore 84.5 mm × 0.5 mm) | 2.80 |
| Deck gap volume (bore 84 mm × 0.5 mm) | 2.77 |
| Required piston dish/dome | 54.76 − 40.0 − 2.80 − 2.77 = 9.19 cc |
| Piston dish volume (incl. valve reliefs) | 9.2 cc |

- Deck clearance: 0.5 mm (piston crown recessed below deck at TDC)
- Squish clearance: 0.5 mm (gasket) + 0.5 mm (deck gap) = 1.0 mm
...
- Supply: Main gallery with check valves opening at ≥ 2.5 bar
...
*(Unchanged – solid bucket valvetrain retained.)*
...
- Squish clearance: 1.0 mm (0.5 mm gasket + 0.5 mm deck gap)
...
- Separate LT circuit for intercooler
...
*(Left as-is; only electronics section included for completeness.)*
...
- 48 V boost for GDI injectors (LT8331, capacitor bank 1000-2000 µF)
- Switched 12 V relay for ignition coil supply

## 13.6 Next ECU Steps

- Finalize pin assignment table for STM32G474RE
- Create detailed power supply schematic
- Design analog input conditioning (VR, knock, wideband, temp)
- Design output drivers (ignition IGBTs, GDI peak-and-hold)
- PCB layout for each modular board
